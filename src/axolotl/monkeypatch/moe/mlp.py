...
class FusedExperts(nn.Module):
    def __init__(
        ...
        # Compute the dimensions based on the input experts
        expert_in_dim = experts[0].w1.weight.shape[1]
        expert_out_dim = experts[0].w2.weight.shape[0]
        
        self.experts = ParallelExperts(num_experts, expert_in_dim, 2 * ffn_dim, device=device)
        self.output_experts = ParallelExperts(num_experts, ffn_dim, expert_out_dim, device=device)
