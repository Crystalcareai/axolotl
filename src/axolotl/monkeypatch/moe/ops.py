...
def _scatter2scatter_configs():
   return [
       triton.Config({'BLOCK_N': BLOCK_N, 'BLOCK_K': 32}, num_stages=4, num_warps=8),
   ]
...
def _scatter2scatter(
   ...
   E_idxs = tl.load(expert_idxs_ptr + M_block, mask=M_block < FAN_OUT_M, other=E)
   E_idx = tl.min(E_idxs)
   E_mask = E_idxs == E_idx
   ...
   for K_block_id in range(0, iters):
       ...
       # Cast x and w to the compute_dtype
       x = x.to(compute_dtype)
       w = w.to(compute_dtype)
       
       # Perform the computation in the compute_dtype
       acc_result = tl.dot(x, w, allow_tf32=allow_tf32)
       
       # Cast the result back to the original data type of acc
       acc += acc_result.to(ACC_TYPE)
...
def scatter2scatter(X, W, sorted_expert_idxs, sorted_scattered_idxs, k,
   ...
   X = X.to(device='cuda')
   W = W.to(device='cuda')
   sorted_expert_idxs = sorted_expert_idxs.to(device='cuda')
   sorted_scattered_idxs = sorted_scattered_idxs.to(device='cuda')
   padded_block_idxs = padded_block_idxs.to(device='cuda')
